...
#   gate-before [args]        arm the implement-next run gate (print gate-armed <key>)
#   gate-verdict [args]       report the run-gate verdict (attributed or observe-only)
#   stack-base [args]         print one change's effective base branch (stacked-changes resolver)
#   stack-children [args]     list one change's stacked descendants (id, status, PR)
#   stack-closeout [args]     promote a merged stack root's stacked-merged descendants to done
#
...
WRAPPED_OPS="docket-status board-refresh archive-change terminal-publish cleanup-feature-branch github-mirror sync-integration-branch render-change-links render-artifact-backlink render-adr-index render-learnings-index adr-checks board-checks reclaim-claims mint-stub runner-dispatch mark-publish-deferred backfill-change-types verify-run gate-before gate-verdict stack-base stack-children stack-closeout"
