...
# set_field FILE KEY VALUE — replace a scalar in the first ---…--- block only (portable sed; no
# GNU-only `sed -i`, per AGENTS.md shell-portability and frontmatter-anchor rules).
set_field(){
...
write_stub(){
  ...
  sed -E 's/[[:space:]]+#.*$//' "$tmp" > "$tmp2" && mv "$tmp2" "$tmp"
  set_field "$tmp" id "$id"
  set_field "$tmp" slug "$SLUG"
  set_field "$tmp" title "$TITLE"
  set_field "$tmp" created "$TODAY"
  set_field "$tmp" updated "$TODAY"
  set_field "$tmp" discovered_from "[$FROM]"
  {
    cat "$tmp"
    printf '\n## Artifacts\n\n'
