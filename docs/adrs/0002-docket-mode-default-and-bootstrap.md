...
## Update — 2026-07-11 (change 0054)

**Decision 3 still stands.** Change 0053 introduced `skills/docket-convention/references/terminal-close-out.md`, and change 0054 adopted it as the documented home for the terminal-transition close-out **sequence** — archive, re-render `## Artifacts`, terminal-publish, cleanup, board — plus the per-caller failure posture for each step. That reference is now the single source for ordering and posture. This does not reverse or supersede Decision 3: `docket-finalize-change` remains the owner of *when* to run each step, keeps the *Harvest learnings* step (which is not part of the reference), and still authors the human-legible commit messages. Only the doc home for the shared *sequence* moved out of `docket-finalize-change` and into the shared reference; the single-source *principle* is unchanged.
