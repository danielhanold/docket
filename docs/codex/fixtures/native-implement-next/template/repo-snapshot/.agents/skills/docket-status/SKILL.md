...
**Sweep posture:** the read (`status`) never repairs — only the explicit `maintenance.sweep` mutation closes changes out, and only for what already merged. Judge a sweep by its **terminal envelope** (per *Completion barrier* above), then by its individual **`entries`**: a top-level `applied` means some work applied, not that every entry succeeded, so inspect each entry's `disposition` and `reason` even under it. A per-change failure **logs its entry and continues**; one item's `blocked` / `failed` / `unknown` never stops the independent items. **Preserve `unknown`:** never treat it as done, and never run a destructive follow-up behind it. **Surface, don't repair:** report the `blocked` / `failed` / `unknown` entries and any `findings` a human must see, take no unsolicited action, and read any retry advice off that entry's own schema-owned `reason`. One drift self-heals: a stale `## Artifacts` block surfaces later as an `artifact-links-stale` finding that authorized `docket repository migrate` re-renders. Sweep recovery stays **distinct from `docket-finalize-change`'s merge authorization** — the sweep only archives already-merged PRs, never merges, and never overrides approval. This log-and-continue posture is **deliberately divergent from `docket-finalize-change`'s** abort-and-report posture — the close-out sequence is shared, the failure posture is not.
