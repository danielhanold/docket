...
- To unblock change 444's resume, the epoch record this same run left `cancelling` (unable to
  ...
  unrelated pre-existing drive, never itself. This is a one-off unblock, not a remedy; keep it as
  a second fixture shape for the cancellation-side regression test above (an epoch record stuck
  `cancelling` behind unrelated `linkage-unresolved` findings, hand-repaired to `cancelled`).
