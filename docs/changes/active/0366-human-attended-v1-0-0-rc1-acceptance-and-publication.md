...
1. **Establish the release source.** Verify change 0318 is merged, the integration branch is clean,
...
3. **Package once.** Dispatch change 0317's candidate workflow for `v1.0.0-rc1` at the exact 0318
...
|---|---|
| Source identity | Exact merged commit, Go/toolchain identity, 0318 suite command and outcome, inventory result, budget findings |
...
- Do not run a maintenance sweep during the bounded interval between merging 0318 and completing
  this release metadata closeout.

## Open questions

- What exact operator checklist and quiescence proof will bracket the post-0318 release window?
...
- What are the exact retry rules after a matching tag exists but the Release or asset set is only
  partially present?
- How are 0318's terminal state and this successor's metadata sequenced so no normal sweep closes
