...
created: 2026-05-30
updated: 2026-05-30
depends_on: [4]           # change ids whose PR must be merged (they reach `done`) before this can start
...
2. **Claim** — re-read the manifest after the pull (avoid double-claim), set `status: in-progress` + `branch`, `updated`; commit and push on `metadata_branch`. *The lock is a **compare-and-swap**: re-read `status` under a freshly-pulled tree, claim, push; a non-fast-forward rejection means someone else moved — `pull --rebase` and **re-read before retrying** (after a rebase your push would succeed, so the re-read, not the push, is the actual guard).* (No worktree yet — step 4.)
...
The whole point: run the **human-driven producer** (`docket-new-change`, brainstorming new build-ready changes) and the **autonomous implementer** (`docket-implement-next` in a loop, draining them to PRs) in parallel — the human's interactive time is all at change creation; draining is hands-off.
...
**The atomic claim** (either mode) is a **compare-and-swap**: `git pull --rebase` → **re-read `status`** → if still `proposed`, set `in-progress`, commit, and **push before building**; on a non-fast-forward rejection, `pull --rebase` and **re-read again** (mandatory — after the rebase the push itself would succeed, so the re-read is the real arbiter) before retrying or picking another change. Because the arbiter is the re-read against the freshly-pulled remote, **don't run two agents against one shared local clone** (no remote to pull from between them); give each agent its own clone.
...
- **Claim race (two implementers):** mutual exclusion is a **compare-and-swap** — `git pull --rebase`, **re-read `status`** immediately before claiming, push; a non-fast-forward rejection forces `pull --rebase` + **re-read again** (mandatory — after the rebase the push would succeed, so the re-read is the arbiter) + retry or pick another; skip non-`proposed`; lowest-eligible-id ordering. Worst case, two agents claim different changes — never the same one. (Two agents must not share one local clone — §8.)
- **Concurrent archive (finalize vs sweep):** the `implemented → done` move is idempotent — each path re-pulls, re-reads `status` **on `metadata_branch`**, and no-ops if already `done`; the `YYYY-MM-DD` prefix is taken from the **merge commit** (the kill-commit for `killed`) so concurrent paths agree on the filename; the metadata push is retried on a non-fast-forward race.
...
- **Open items to settle during implementation:** what the default `scan` candidate-source list contains (its *location* is settled — a `.docket.yml` knob, §5).
