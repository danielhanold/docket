...
- **Concurrent archive (finalize vs sweep):** the `status` re-read on `metadata_branch` handles the *non-concurrent* case (no-op if already `done`). In a genuine race — both paths read `implemented` and both archive — safety rests on **determinism**: the filename uses the **merge-commit date** (kill-commit for `killed`) and the archive commit writes **no nondeterministic field** (`updated:` is set to that same date, not `now()`), so both produce a **byte-identical add** and the losing push's `pull --rebase` resolves the add/add cleanly. The push is retried on a non-fast-forward race.
...
- **Open items to settle during implementation:** what the default `scan` candidate-source list contains (its *location* is settled — a `.docket.yml` knob, §5); and the **stale-`in-progress` threshold** default (currently `3 days`, a future `.docket.yml` knob — §7.4).
