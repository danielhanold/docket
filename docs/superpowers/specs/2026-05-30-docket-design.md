...
5. **Lifecycle is a `status` field + a single archive move.** Seven states; `active/` holds non-terminal, `archive/` holds terminal (`archived`, `killed`). The only physical file move happens once, on the terminal transition — so parallel agents never race on `git mv`. Finer state (blocked/deferred) is a field value, not a directory.
...
docket/
  skills/
    docket-propose/SKILL.md      # producer: idea → proposed change (+ opt-in scan mode)
    docket-next/SKILL.md         # implementer: pick → reconcile → build → PR → stop
    docket-status/SKILL.md       # board render + merge-sweep janitor + health checks
  references/
    CONVENTION.md                # dir layout + manifest schema + 7-state lifecycle (single source)
  templates/
    change.md                    # the change-file stub
  README.md                      # what docket is, install, the one prerequisite (superpowers)
```

Each skill is a thin orchestrator. The shared `references/CONVENTION.md` states the directory layout, manifest schema, and lifecycle *once*; all three skills point at it via progressive disclosure rather than restating it.
...
status: proposed          # proposed | in-progress | blocked | deferred | implemented | archived | killed
priority: 2               # 1 = highest
created: 2026-05-30
updated: 2026-05-30
depends_on: [4]           # change ids that must reach `archived` before this can start
...
```
                         ...
                         ▼                                          │
  proposed ──claim──▶ in-progress ──PR open──▶ implemented ──merge+sweep──▶ archived
     │                    │                                                  (archive/)
...
| `archived` | PR merged, filed away (happy terminal) | `archive/` |
...
3. **Draft** — write `active/<id>-<slug>.md` from `templates/change.md`: frontmatter (`status: proposed`, dates, priority) + body.
...
0. **Sync & sweep** — `git pull`; invoke the `docket-status` merge-sweep so any `implemented` change whose PR has merged is archived first (self-cleaning loop).
...
- **Merge sweep** — for each `implemented` change, check via `gh` whether its `pr` merged → move the file to `archive/YYYY-MM-DD-<id>-<slug>.md`, set `status: archived`, commit. Closes the loop after a human merge.
...
- The **convention itself** (`CONVENTION.md` + the file layout) is pure data + prose and is therefore portable by definition, independent of any harness.
