...
Base paths are configurable knobs; defaults `docs/changes/` and `docs/adrs/`. Both trees are docket-managed; ADRs are never archived (no `archive/` under `docs/adrs/`). A third knob, `metadata_branch` (default `main`), controls where lifecycle/board/ADR commits land — see §8. All three live in an optional committed `.docket.yml` (below); omit it and you get the defaults.
