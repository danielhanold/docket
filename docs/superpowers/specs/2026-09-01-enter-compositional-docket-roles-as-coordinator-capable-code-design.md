...
> ↩ **[Change 0393 — Enter compositional Docket roles as coordinator-capable Codex root threads](https://github.com/danielhanold/docket/blob/docket/docs/changes/archive/2026-09-09-0393-enter-compositional-docket-roles-as-coordinator-capable-code.md)**
<!-- docket:backlink:end -->
...
## 2026-09-08 amendment — feature-scoped child entry is worktree-anchored

This amendment supersedes three parts of the original design: Architecture section 4's rule that
every direct child uses native named-agent control, the Out-of-scope exclusion for worktree scopes,
and the corresponding success criterion for an unqualified direct `docket-plan-writer` dispatch.
Those clauses assumed native child launch preserved the existing worktree boundary. Finalizing
change 0349 through the change-0393 root-entry implementation disproved that assumption: the
coordinator ran in change 0393's worktree, the owned rebase conflicted in change 0349's worktree,
and the resolver observed no live unmerged index entries because native named-agent dispatch had no
working-directory channel.

### Revised decision

Treat `worktree-scope: feature|metadata` as a required typed fact in the shared agent inventory and
the Codex role contract. Codex dispatch uses both scope and launch posture:

- Parent-facing `root-coordinator` roles continue through foreground `agent.enter` with the caller's
  absolute cwd.
- Metadata-scoped ordinary children continue through native named-agent dispatch.
- Feature-scoped children enter through the same foreground app-server mechanism with an explicit
  `--worktree <absolute-path>`. The operation validates that the target is a registered non-primary
  worktree of the same repository and uses that path as the child thread's actual cwd.

The feature worktree is runtime authority supplied by the owning workflow after its typed context
or workspace operation resolves it. It is not inferred from the caller's cwd, a role-name pattern,
the request's change id, or prose. The child request remains byte-identical; change identity stays
inside the workflow payload rather than becoming an `agent.enter` flag.

Codex's generated dispatch policy marks feature-scoped roles from the typed inventory and requires
the worktree-anchored entry path. A feature-scoped role also checks at startup that its actual cwd
equals the absolute worktree named in its request. A missing or mismatched path halts before reads,
writes, Git staging, or authored output. This second check makes an accidental ordinary native
launch fail visibly instead of inspecting the coordinator's tree.

This is a narrow app-server entry reuse, not a return to a generic shell runner, another harness,
`codex exec`, or parent relay. The same installed role contract still supplies instructions, model,
effort, and skills, and `agent.enter` still waits for the role's foreground terminal return.

### Enforcement and tests

- Inventory tests require and validate `worktree-scope` on every built-in agent and mutation-test a
  missing or invalid declaration. No hand-maintained role-name list may select feature roles.
- Root-entry CLI and protocol tests prove that a feature role refuses a missing, primary,
  foreign-repository, or nonexistent worktree and that `thread/start.cwd` equals the validated
  feature path.
- Generated-policy tests prove metadata children retain native dispatch while every feature child
  is routed through worktree-anchored entry. Reverting that routing must fail the test.
- A production-shaped cross-worktree regression starts the coordinator in worktree A, creates a
  live conflict in worktree B, and requires the real resolver role to observe B's unmerged index.
- Inventory-derived coverage applies the same launch rule to plan writer, all build profiles, all
  review rungs, rebase resolver, and integration repair.
- Regenerate Codex role files, embedded assets, capability signatures, and maintained dispatch
  policy from their authoritative sources, then run the complete repository build gate.

ADR-0103's statement that all ordinary roles use native child dispatch no longer describes the
safe Codex boundary. Record a superseding ADR: metadata-scoped ordinary roles remain native
children, while feature-scoped roles require verified worktree-root entry.
