...
- Both phases: source must `document.Parse` cleanly (else `results-doc-malformed`); there must be an H1 (`# ` line with nonempty text) outside fenced code and outside managed `docket:*` blocks (else `results-title-missing`); no unfilled authoring placeholder (else `results-placeholder`). A placeholder is (a) a `placeholderTokenRE`-style token (`TBD|TODO|FIXME|TKTK|XXX|PLACEHOLDER` as a word — reuse the same pattern; do not import the plan's regexp, declare a local one or export the existing one) outside fences, or (b) a line whose text — after stripping leading `#` markers, list markers, and whitespace — begins with `<` and is not an HTML comment (`<!--`) or autolink (`<http`). This catches the template's `<What was delivered...>` scaffolding and `### <Finding>` headings.
