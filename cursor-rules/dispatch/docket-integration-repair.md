...
mechanism. The prompt must include the red-test output, the base it was rebased onto, the
feature worktree the rebased branch is checked out in, and the repair-attempt budget
(`finalize.repair_max_attempts` from the prepared context) — this agent is feature-scoped, and a
delegated dispatch that names no worktree is refused. The agent writes a minimal fix within the
dispatched repair-attempt budget and never weakens tests.
