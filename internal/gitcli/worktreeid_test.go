...
// TestDiscoverWorktreeContainingIdentity proves DiscoverWorktree resolves the
...
func TestDiscoverWorktreeContainingIdentity(t *testing.T) {
	requireGit(t)
	c := newRealClient(t)
...
// TestDiscoverWorktreeThroughSymlink proves a path reached through an extra
...
func TestDiscoverWorktreeThroughSymlink(t *testing.T) {
	requireGit(t)
	c := newRealClient(t)
	ctx := context.Background()
	r := newMainModeRepos(t)
...
// TestDiscoverWorktreeRejections asserts the typed failure kinds: an empty path
// (invalid-request), a plain non-repo dir, and a bare repository (both
// invalid-repository).
func TestDiscoverWorktreeRejections(t *testing.T) {
	requireGit(t)
	c := newRealClient(t)
	ctx := context.Background()
	r := newMainModeRepos(t)
