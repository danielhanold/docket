...
// prior install owns. It plans only: nothing here writes to the filesystem
// apart from Detect's read-only stat.
package claude
...
// in ~/.claude/CLAUDE.md. Parent-facing routing instructions now live in a
// repository's own CLAUDE.md, never a personal global one, so Plan omits this
...
// from a freshly rendered body, so no interior is rendered here.
