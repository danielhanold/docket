...
func TestLeafValidators(t *testing.T) {
	...
	cases := []struct {
		...
		{"build max attempts zero", "build.max_attempts", "0", nil, CodeInvalidValue},
		{"build max attempts negative", "build.max_attempts", "-1", nil, CodeInvalidValue},
		{"build max attempts string", "build.max_attempts", `"four"`, nil, CodeInvalidType},
		{"build max attempts fraction", "build.max_attempts", "3.5", nil, CodeInvalidType},
		{"build max attempts bool", "build.max_attempts", "true", nil, CodeInvalidType},
		{"run max attempts explicit", "run.max_attempts", "5", 5, ""},
		{"run max attempts one", "run.max_attempts", "1", 1, ""},
		{"run max attempts zero", "run.max_attempts", "0", nil, CodeInvalidValue},
		{"run max attempts negative", "run.max_attempts", "-2", nil, CodeInvalidValue},
		{"run max attempts string", "run.max_attempts", `"two"`, nil, CodeInvalidType},
