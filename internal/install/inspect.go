...
func inspectFile(t Target, info fs.FileInfo, rec TargetRecord, hasRec bool, legacy LegacyReproducer) (Inspection, error) {
	if info.Mode().IsRegular() {
		...
		return conflict(t, ReasonOwnershipConflict), nil
	}
...
func inspectManagedBlock(t Target, info fs.FileInfo, rec TargetRecord, hasRec bool) (Inspection, error) {
	...
	if err != nil {
		...
		return conflict(t, ReasonManagedBlockInvalid), nil
	}
...
func conflict(t Target, reason string) Inspection {
	return Inspection{Target: t, Disposition: DispositionConflict, Reason: reason}
}
