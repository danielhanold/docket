...
For a small mechanical change with no real design questions: skip the brainstorm — no spec, still build-ready. It follows Brainstorm mode's steps 1 (Allocate), 3 (Scan related context), and 4 (Create the proposed change) unchanged, then step 5 grooms with `outcome: trivial` instead of `spec`: `docket change groom --repo-dir .docket --request <request-file>` with the pinned `path` + `version` and an owned-section edit carrying the tightened body as the trivial rationale — the transaction sets `trivial: true` + `updated:`, re-renders the `## Artifacts` block, and re-renders the inline board atomically (no spec file, no separate Board pass). Only step 2 (Brainstorm) is skipped.
