...
4. **Draft the change** — write the thin `active/<id>-<slug>.md` from `change-template.md`: frontmatter (`status: proposed`, `spec:`, `type:` = one configured `change_type` — REPLACE the template's comment, an unfilled line reads as neither a type nor `untyped`, `created`/`updated` = UTC today (the UTC date of the commit), priority default `medium`) + a PM-altitude why/what/scope body distilled from the brainstorm. Design detail lives in the linked spec, NOT here. When the human says this work builds on another change's **unmerged** branch, set `stacked_on: <parent id>` and **read [`../docket-convention/references/stacked-changes.md`](../docket-convention/references/stacked-changes.md) now (blocking)** first — stacking changes how the change is built, merged, and closed out. When the human provided rich initial context and says the change may be designed without them, set `auto_groomable: true` at draft time — `docket-auto-groom` will carry it to build-ready. Otherwise leave the field unset (it inherits the repo's `auto_groom` default). When the human's request names a branch prefix ("use the `hotfix/` prefix"), record the normalized scalar `branch_prefix: hotfix` — strip one presentation-only trailing slash, but **refuse and ask the human** on a slash-embedded or `refs/`-qualified value rather than repairing it. Claim consumes this field at mint time; it never searches prose for naming clues.
