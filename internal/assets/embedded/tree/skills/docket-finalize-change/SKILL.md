...
- `reserved` — authorizes exactly ONE `docket-rebase-resolver` dispatch (foreground, at the model/effort its wrapper resolves), naming the feature worktree in the payload and including the returned `reservation` token; the resolver echoes it back as the report's `resolver_reservation` field. It edits only the conflicted regions in the returned workspace and returns a versioned `ResolverReport` (fields in step 3 of *The two agents* in `references/gate-failure.md`) — it never runs the rebase mechanics or the suite.
