...
**Reaching docket's operations.** Maintained skills reach every docket operation through the native `docket` binary on `PATH`, but never by hard-coding its argv: each skill resolves the executable spelling from the capability catalog (`docket capabilities --json` — the one spelling this convention permits a skill to hard-code) and constructs the invocation from the entry for the semantic operation it names (`repository.prepare`, `maintenance.sweep`, `status`, `change.*`, `context.*`, `adr.*`, `artifact.backlink`, …) — never a shell facade, and never a `DOCKET_*` transport variable resolved into a command. The two bootstrap remedies a **human** types — `docket repository migrate` and `docket repository init` — are the deliberate exception: they run before any catalog exists to resolve against, are human-initiated, and are never an agent runtime invocation. The deterministic helper scripts this convention still names (for their frozen and `main`-mode contracts) live in the docket clone's `scripts/` directory, NOT in the consuming repo; read a script's co-located `scripts/<name>.md` contract for its internals. If the `docket` binary is missing from `PATH`, the install is broken — stop and fix it, never silently degrade to hand-worked operations. Every env var docket introduces is **DOCKET_-namespaced**.
...
2. **Capability bootstrap.** If the current agent context does not already carry a validated capability catalog, run `docket capabilities --json` **as its own Bash call** — the one executable docket CLI spelling this convention permits skills to hard-code — and validate the response **fail-closed before any other docket invocation**:
   ...
   - **refuse** (stop and surface the diagnostic) on any of: `protocol_version` ≠ 1 or an unsupported `capability_version`; a malformed envelope; duplicate `id`s; an effect outside the closed vocabulary (`read` | `local-write` | `metadata-write` | `external-write` | `process-control`); an entry with a missing/empty `argv`; an absent or empty `commands` array.
...
This 2×2 is the spec the config resolver computes as its `BOOTSTRAP=` verdict — `PROCEED` (migrated or main-mode), `STOP_MIGRATE` (existing single-branch or half-migrated), or `CREATE_ORPHAN` (fresh). `repository.prepare` enforces that verdict **fail-closed** internally and surfaces it as the Step-0 disposition the *Step-0 preamble* acts on: an `applied`/`no-op` when the verdict proceeds, a `refused`/`error` whose remedy names the human-typed `docket repository migrate` or `docket repository init` otherwise. Probe mechanics, the read-only export default, and the guarded bootstrap write path live in `repository.prepare`. The guard is a no-op in `main`-mode; the migration itself lives in the human-typed `docket repository migrate`.
