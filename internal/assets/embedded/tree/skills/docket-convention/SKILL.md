...
3. Act on the disposition: `applied`/`no-op` → continue. A `refused` on a **legacy single-branch** layout → refuse and point at `docket repository migrate` (or the human-initiated `migrate-to-docket.sh` setup script, never an agent runtime invocation). A `refused` on a **fresh repository** (once, human-attended) → run `docket repository init`, then re-run `docket repository prepare`. Any other `refused`/`error` (dirty, ahead, diverged, foreign, ambiguous, or an unresolved probe) → surface the finding's remedy and stop; it is a human's to resolve.
