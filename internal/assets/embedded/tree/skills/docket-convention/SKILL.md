...
**Three-boundary separation.** A dispatch spans **three non-overlapping places**: (1) the **workflow edge** (child, posture, payload, receipt) in the **caller skill**; (2) the **role behavior** (what the agent *is* and does) in `agents/docket-*.md`; (3) the **harness launch mechanics** only in the harness adapter (`internal/harness/<harness>`) and its generated surfaces. Boundaries (1)–(2) stay **harness-neutral**: *dispatch the named agent*, never product syntax.
