#!/usr/bin/env bash
# tests/test_go_race_transaction.sh — the race shard for internal/repository/transaction
# (change 0309).
#
# WHY THIS IS A SIBLING SHARD of tests/test_go_race.sh and not a fifth check
# inside it. The whole-module race gate (`go test -race ./...`) grew past the
# point where one file could carry it under the 60s hard ceiling in
# tests/test_runtime_budgets.sh: internal/gitcli alone runs ~54s instrumented,
# and change 0309's internal/repository/transaction package adds ~36s more of
# real-git, multi-clone concurrency fixtures under the detector. Folded together
# they measured 57-58s standalone serial — worst 58, which the sizing rule (next
# multiple of 5 plus a 5s margin) puts at 65, ABOVE the ceiling with no next
# raise available. The ceiling is a relief counter; raising it is the evasion it
# exists to catch. Sharding is the sanctioned answer, exactly as change 0324 cut
# tests/test_sync_agents_runners.sh: the new heavy package moves into its own
# budgeted row, and tests/test_go_race.sh runs the complement. The two files'
# package sets partition `go list ./...` exactly, and that partition is asserted
# in tests/test_go_race.sh so a future package can never silently fall out of the
# race gate.
#
# WHY THE TRANSACTION PACKAGE is the one pulled out (rather than gitcli): it is
# the package this change introduces, so isolating it keeps the shared row from
...
# The detector's verdict for exactly the transaction package. A race is reported
# on stderr and turns the exit non-zero, so the captured output is replayed on
# failure rather than summarized — the WARNING block names the two conflicting
# stacks and is the whole diagnostic. The package selector matches the same
# import path tests/test_go_race.sh EXCLUDES from its own run; the completeness
# guard there proves the two sets partition `go list ./...`.
race_out="$(go test -race ./internal/repository/transaction/ 2>&1)"
race_rc=$?
assert "go test -race ./internal/repository/transaction/ passes" '[ "$race_rc" -eq 0 ] || { printf "%s\n" "$race_out" >&2; false; }'
