...
assert "emit: opens with new start marker" 'printf "%s\n" "$BLK" | sed -n 1p | grep >/dev/null -F "# docket:start (managed by docket — do not hand-edit)"'
assert "emit: closes with new end marker"  'printf "%s\n" "$BLK" | tail -1 | grep >/dev/null -xF "# docket:end"'
assert "emit: core .docket/"               'grep <<<"$BLK" -qxF ".docket/"'
assert "emit: core .worktrees/"            'grep <<<"$BLK" -qxF ".worktrees/"'
assert "emit: core settings.local.json"    'grep <<<"$BLK" -qxF ".claude/settings.local.json"'
assert "emit: .docket.local.yml"           'grep <<<"$BLK" -qxF ".docket.local.yml"'
assert "emit: claude agents pattern"       'grep <<<"$BLK" -qxF ".claude/agents/docket-*.md"'
assert "emit: windsurf agents pattern"     'grep <<<"$BLK" -qxF ".windsurf/agents/docket-*.md"'
assert "emit: codex TOML wrapper pattern"   'grep <<<"$BLK" -qxF ".codex/agents/docket-*.toml"'
assert "emit: codex .md pattern still present (constant, all tokens)" 'grep <<<"$BLK" -qxF ".codex/agents/docket-*.md"'
assert "block ignores generated opencode agent definitions" \
  'grep <<<"$BLK" -F -x -q -- ".opencode/agents/docket-*.md"'
assert "emit: cursor dispatch rule"       'grep <<<"$BLK" -qxF ".cursor/rules/docket-dispatch.mdc"'
assert "emit: every non-marker line is docket-scoped (starts with . )" \
  '! grep <<<"$BLK" -v "^#" | grep >/dev/null -vE "^\."'
...
assert "emit: .docket/ precedes .docket.local.yml" \
  '[ "$(grep <<<"$BLK" -nxF ".docket/" | cut -d: -f1)" -lt "$(grep <<<"$BLK" -nxF ".docket.local.yml" | cut -d: -f1)" ]'
...
assert "ensure: idempotent run prints no UPDATED/UPGRADED notice" '! grep <<<"$err2" -qiE "updated|upgraded"'
...
assert "upgrade: announces the upgrade"        'grep <<<"$upg_err" -qi "upgrad"'
rm -rf "$SBX"
...
assert "dangling-new: warns unterminated/corrupt" 'grep <<<"$dg_err" -qiE "untermin|corrupt"'
...
assert "dangling-legacy: warns unterminated/corrupt" 'grep <<<"$dl_err" -qiE "untermin|corrupt"'
...
assert "out-of-order: warns corrupt/unterminated"    'grep <<<"$oo_err" -qiE "untermin|corrupt|malformed|out.of.order"'
...
assert "out-of-order legacy: warns"                  'grep <<<"$ool_err" -qiE "untermin|corrupt|malformed|out.of.order"'
...
assert "dedup: advisory logged for outside bare entries" 'grep <<<"$adv_err" -qi "safe to delete"'
