...
assert "auto-create seeds a SINGLE_SELECT Status field with every active status" \
  'grep -qE "project field-create .*--data-type SINGLE_SELECT" <<<"$autoout" && grep -qF "in-progress,proposed,blocked,deferred,implemented,stacked-merged" <<<"$autoout"'
