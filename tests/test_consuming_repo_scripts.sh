...
grep <<<"$err" -qF "run docket/install.sh" && ok "fail-loud message carries the remedy" \
...
grep <<<"$evalerr" -qF "run docket/install.sh" && ok "eval-site: unset var surfaces the remedy on stderr" \
