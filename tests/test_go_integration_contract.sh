...
# The walk is ROOTED at the literal `internal` and `cmd` pathspecs — every Go integration
# test in this module lives under those package roots — so it is provably bounded away from the
...
# bound, not a hand-maintained package allowlist: any internal/**/…_integration_test.go or
# cmd/**/…_integration_test.go is still discovered automatically. Capture-then-filter (never producer|grep under
# pipefail); the suffix regex is basic ERE, /usr/bin/grep-safe.
tracked_go="$(git ls-files -- internal cmd)"
int_files="$(grep -E -e '_integration_test\.go$' <<<"$tracked_go")"
assert "at least one Go *_integration_test.go exists under the bounded roots" '[ -n "$int_files" ]'
