...
**`github` surface** — the one-way Issues + Projects v2 mirror (per the convention's *GitHub board mirror* definition). Invoke the deterministic `scripts/github-mirror.sh` against the change files, **best-effort**: it needs network + `gh` auth, it never aborts the pass, and it self-heals next run. The script upserts one issue per change (keyed on `issue:`), reconciles the `docket:` label set, sets close state/reason, and best-effort-syncs Projects; on a fresh mint it prints `issue-minted <id> <number>` lines — record each into the change file's `issue:` on `metadata_branch` (the script does no git writes), and the `github_project` write-back (first-sync mint) lands in `.docket.yml` on the default branch. Both metadata writes follow the normal push discipline; a `gh`/network failure logs and continues.
