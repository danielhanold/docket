...
- **`learnings index clean` / `learnings index changed pushed` / `learnings index changed push-failed` / `learnings index blocked-wedged-tree`** — the derived index's self-heal, on a full pass only; see *Learnings* below for the write shape. The last is the wedged-tree token again: nothing was committed or pushed, it is not retryable, and the index self-heals next pass once a human has cleared the operation.
