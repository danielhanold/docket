...
- **`harvest <id> <path>` lines** — for each, run the harvest-learnings procedure (the *Harvest learnings* step in `docket-finalize-change`'s SKILL.md is its single source — invoke it by reference, do not reimplement it here). Best-effort: log and continue on failure, never abort the rest of the pass for it. The same step's auto-capture leg (convention: *Auto-capture*) applies here too, with the cap scoped **per swept change**; also best-effort — log and continue.
