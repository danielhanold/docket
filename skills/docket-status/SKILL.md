...
4. **Mermaid dependency graph** built from `depends_on` edges; `done` nodes tinted with `classDef done fill:#d3f9d8;`. Renders on GitHub and Markhaus; degrades gracefully in plain CommonMark.

5. **Collapsible `<details>` "Done" section** for archive entries.
...
3. **Merged → ARCHIVE IDEMPOTENTLY:**
   ...
   f. **Remove the merged feature branch + worktree**, provenance-guarded: only auto-remove worktrees under a known `.worktrees/` path — same guard as `superpowers:finishing-a-development-branch`.
...
- **Stale `in-progress` past the build step** — the planned branch is gone, or exists but has had no commits in N days (default 3). A just-claimed change with a `branch:` value but no branch yet created is **not** stale.
