...
Trust its exit code for the pass as a whole: `0` is the pass completing (`board off`, `pass ok`, findings, `sweep-failed`, `sweep-skipped`, `board *-failed`, and `judgment` lines on stdout are all normal outcomes, not errors); non-zero is a hard error — config export failure, an unusable `BOOTSTRAP` verdict, an unusable metadata worktree, a bad CLI argument, or a resolved `board_surfaces` that came back empty or with `none` combined with another surface (both a wiring bug, change 0071) — surface the stderr diagnostic and stop rather than improvising a fix. **The Board pass specifically is narrower than that**: key on its stdout report line, never its exit code alone (the convention's *Board refresh on status writes* is the single source of that contract). A pass that exits `0` but prints no `board …` line at all, or ends its Board pass on `board inline failed`, `board <token> unknown`, `board github failed`, or `board inline changed push-failed`, has ALSO failed at that step — the overall exit code being `0` does not mean the board landed.
...
**Sweep posture:** per-change failures **log the error and continue to the next change**; whether *that* change's own remaining close-out steps are abandoned depends on which step failed and how. The next sweep self-heals idempotently **only for a failure before the archive step** (`sync pull-failed` or `archive script-error`) or a `cleanup` failure — those retry cleanly next pass. A `sweep-failed` at `render-change-links` with reason **`skipped-publish`** (the renderer itself exited non-zero) — or at `terminal-publish` — **does** abandon the remainder of this change's close-out, leaving it **archived but its terminal record unpublished**, invisible to future detection (which only scans `active/*.md`); that's the one case needing a manual `docket.sh terminal-publish --id <id> --enabled true` follow-up. A `sweep-failed` at `render-change-links` with reason **`commit-failed`** or **`push-failed`** (step 6a, change 0075) is different: **report-and-continue**, not abandonment. The close-out already completed — `terminal-publish` and `cleanup` still ran, and the pass still emits `swept`/`harvest` for the change — only the cosmetic `## Artifacts` link block on the archived file is stale on `metadata_branch`. No manual `terminal-publish` is needed there; it self-heals on the next pass that re-renders the file. Don't let a `sweep-failed render-change-links` line alone imply the record is unpublished — check the reason, and cross-check against whether the same pass also emitted `swept`/`harvest` for that id. The knob narrows only the `terminal-publish` leg — under `terminal_publish: false` that step is a no-op that cannot fail — but `render-change-links` can still fail in such a repo, leaving a stale `## Artifacts` block on `metadata_branch`; the follow-up there is a manual re-render, not a publish. This is **deliberately divergent from `docket-finalize-change`'s** abort-and-report posture — the sequence is shared, the failure posture is not. A failed `docket.sh render-change-links` follow-on skips publish (a stale `## Artifacts` block is never published).

### Learnings

Runs on a **full pass only** — never under `--board-only` — and is gated FIRST on `learnings.enabled` (default `true`), before the cap is ever consulted: disabled means the renderer is never invoked, no advisories are computed, and every existing `learnings/` file is left byte-untouched — a read/write gate, never a purge. Enabled, the pass self-heals the derived index (`learnings/README.md`, rendered by `render-learnings-index.sh` — contract: `scripts/render-learnings-index.md`) using the identical write shape the board uses for `BOARD.md`: render in place, diff, commit only when the bytes actually changed, push with the same bounded rebase-retry (regenerating through the renderer on a conflict, never a hand-merge). It then surfaces two needs-you advisories, both visibility-only: `learnings over-cap — needs curation` once active findings (`retained` + `candidate`; a `promoted` finding no longer counts — that is the shrink valve) exceed `learnings.cap`, and `learnings promotion-pending <n> — needs you` whenever any finding carries `promotion_state: candidate`. Populating `learnings/` itself is the harvest's job, not this pass's — see the `harvest <id> <path>` hook into `docket-finalize-change` under *Judgment follow-ups* above.
