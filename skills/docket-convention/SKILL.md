...
**Board refresh on status writes.** Any skill that writes a change's `status:` refreshes **each enabled board surface** (the Board pass) immediately after — the `inline` surface regenerates `BOARD.md` by invoking the gated `board-refresh.sh` entry point (it owns the write decision: renders and replaces `BOARD.md` only when `inline` is among the resolved `$BOARD_SURFACES` tokens, otherwise it leaves the file untouched) and commits the result **only if BOARD.md changed** (`git status --porcelain` on it is non-empty) in a separate commit; the `github` surface runs the mirror upsert (best-effort); `board_surfaces: []`, or a configuration that omits `inline` (e.g. `github`-only), makes the `inline` write a no-op — the prior board, if any, is left untouched rather than truncated. The board is a derived view and must never trail the change files.
