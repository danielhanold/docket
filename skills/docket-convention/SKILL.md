...
Each **autonomous** docket skill can run as a model/effort-pinned **subagent** instead of inline at the session model. Seven skills get a wrapper — `docket-implement-next`, `docket-auto-groom`, `docket-finalize-change`, `docket-status`, `docket-adr`, `docket-build-task` (shared by its four profile agents), and `docket-review` (shared by its three rung wrappers); the two **interactive** skills (`docket-new-change`, `docket-groom-next`) stay inline and only surface an **advisory** recommended model/effort at startup (a skill cannot force the session model). `docket-convention` is not an agent — it is injected via `skills:` into every wrapper except eight: `docket-brainstorm-consultant` (ADR-0022), the four `docket-build-*` profile workers, and the three `docket-review-*` rung wrappers. Those **seven** wrapper-bearing exceptions perform no docket metadata operations — a boundary each worker contract's own *Scope* states normatively, since that contract is the only thing they load; the consultant wraps nothing at all (see *Composition*).
