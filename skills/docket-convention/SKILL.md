...
**Board refresh on status writes.** Any skill that writes a change's `status:` refreshes the board immediately after — the **Board pass** — by invoking the one facade call `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/docket.sh docket-status --board-only`. That orchestrator owns the whole decision: it resolves config itself (fail-closed), gates on the enabled surfaces, renders the `inline` surface through the gated `board-refresh.sh` writer, runs the `github` mirror upsert (best-effort), and commits + pushes `BOARD.md` on `metadata_branch` **only if it actually changed** — a separate commit from the `status:` write, with its own rebase-retry. **No surfaces value is ever passed by a skill**: the caller never resolves, spells, or forwards one, which is what makes an unresolved config impossible to mistake for a disabled board. The pass reports its outcome on stdout — `board off` (the board is deliberately disabled, `board_surfaces: []`), `board inline clean` (nothing changed), `board inline changed pushed`, `board inline changed push-failed`, `board github ok`, or `board github failed` — and **callers key on that report line, never on the exit code**. Of that set, `board inline changed push-failed` is the *only* retryable line — every other line is terminal, including the two `github` outcomes, which match none of `pushed`/`clean`/`off` and are not `push-failed` either; a must-land caller re-invokes solely on `board inline changed push-failed` (bounded, then surface the failure), never on an open-ended "until one of these lines appears," or a `board_surfaces: [github]` repo's retry loop would never terminate. A repo with `board_surfaces: []` renders and commits nothing, and a pre-existing `BOARD.md` is left untouched rather than truncated. The board is a derived view and must never trail the change files.
