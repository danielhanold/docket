...
3. Act on the verdict: `PROCEED` → continue. `STOP_MIGRATE` → refuse and point at `migrate-to-docket.sh` (a human-initiated setup script, never an agent runtime invocation). `CREATE_ORPHAN` (fresh repo, once, human-attended) → run `docket repository init`, then re-run `docket.sh preflight`.
