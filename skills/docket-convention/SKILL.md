...
**`build:` — SDD sub-dispatch model IDs (change 0044).** `build.implementer` / `build.reviewer` govern only `docket-implement-next`'s sub-dispatches into `superpowers:subagent-driven-development` (SDD): `implementer` covers the per-task implementer and fix subagents; `reviewer` covers the per-task task-reviewer and the final whole-branch code-reviewer. Each value is a **direct model ID**, passed straight through to SDD's `model:` field — the same harness-neutral passthrough contract the `agents:` block uses, so under Claude Code it is a Claude alias/ID and under another harness (e.g. Cursor) one of that harness's own model IDs; docket does not interpret or validate the string. `build:` is global-able (not a coordination key) and layers like any other config key. An unset role — or an absent `build:` block entirely — defers to SDD's own Model Selection judgment, so this surface is purely additive: today's behavior is exactly the unset case.
