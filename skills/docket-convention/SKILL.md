...
Each **autonomous** docket skill can run as a model/effort-pinned **subagent** instead of inline at the session model. The autonomous skills get a wrapper — `docket-implement-next`, `docket-auto-groom`, `docket-finalize-change`, `docket-status`, `docket-adr`, `docket-build-task` (shared by its four profile agents), and `docket-review` (shared by its three rung wrappers); the two **interactive** skills (`docket-new-change`, `docket-groom-next`) stay inline and only surface an **advisory** recommended model/effort at startup. `docket-convention` is not an agent — it is injected via `skills:` into every wrapper except these: `docket-brainstorm-consultant` (ADR-0022), **`docket-plan-writer`**, the `docket-build-*` profile workers, and the `docket-review-*` rung wrappers. These wrapper-bearing exceptions perform no docket metadata operations — a boundary each worker contract's own *Scope* states normatively; the consultant wraps nothing at all (see *Composition*).
...
docket's workflow steps are **pluggable roles**: the optional `skills:` map rebinds each to any skill name, or to the sentinel `auto`. An unset key defaults to the skill shown — superpowers for `brainstorm`/`plan`/`finish`, docket's own for `build`/`review` (change 0193).
...
resolved as `DUMMY_MODE_ENABLED` / `DUMMY_MODE_PERSONA` / `DUMMY_MODE_SURFACES`) governs it. The eligible
surfaces: `dialogue` and `reports` are **replaced** — written calibrated to the
...
to author any of these surfaces → read [`references/dummy-mode.md`](references/dummy-mode.md)
...
This 2×2 is the spec the config resolver computes as its `BOOTSTRAP=` verdict — `PROCEED` (migrated), `STOP_MIGRATE` (existing single-branch or half-migrated), or `CREATE_ORPHAN` (fresh). `repository.prepare` enforces that verdict **fail-closed** internally and surfaces it as the Step-0 disposition the *Step-0 preamble* acts on: an `applied`/`no-op` when the verdict proceeds, a `refused`/`error` whose remedy names the human-typed `docket repository migrate` or `docket repository init` otherwise. Probe mechanics, the read-only export default, and the guarded bootstrap write path live in `repository.prepare`. The migration itself lives in the human-typed `docket repository migrate`.
