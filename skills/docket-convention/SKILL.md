...
**Composition (change 0017).** `docket-implement-next` dispatches the `docket-status` subagent (step 0) and `docket-adr` (step 6). Both are **foreground** (the parent suspends until the child returns), **unconditional**, and rest on **git state** on `origin/docket`, re-read after a re-sync — for `docket-adr` that is the whole contract, never an in-context return. The `docket-status` step-0 dispatch is **hybrid** (change 0389): beyond those mutations, its return must carry the scoped sweep's terminal protocol-v1 evidence plus a successful post-sweep status read — the parent accepts no bare `completed`, and a first terminal result arriving after the parent advanced is a contract violation, never a dismissable duplicate. That sweep runs at implementation scope (`docket maintenance sweep --scope implementation`) — current merged-work recovery and reclaim gating, with historical cleanup retries left to explicit full maintenance, so startup no longer implies a full historical sweep. `docket-implement-next` also dispatches `docket-plan-writer` (step 4) — foreground and unconditional on the same blocking terms, but hybrid: its receipt (`PLAN_PATH=<path>`) flows back in-context, while the proof is **git state on `<type>/<slug>`** (the committed plan + backlink + `Docket-Plan-Path:` trailer), which the parent verifies before attaching; unavailable dispatch is Tier C, per *Dispatch-capability resolution*. `docket-auto-groom` dispatches `docket-auto-groom-critic` for its adversarial gate on the same terms, but its verdict flows back **in-context as the dispatch's return** — never via git state and never via agent messaging, because a dispatched groom is not registered under its skill name and no name-addressed delivery to it resolves (change 0281). **Foreground means the parent *actively blocks* on the child's return — it may never background a dispatched or forked child and *yield* to await a task-notification** (a forked/subagent skill has no channel to receive one, ADR-0024): yielding returns a **half-done run that the caller reads as `completed`**. Reciprocally, a caller must **not** read a bare `completed` as proof the child finished: it verifies the child's git-state transition and **never adopts or commits a child's uncommitted working-tree files**. For an interactive dispatch that obligation is mechanical: the managed `docket:dispatch` block in each harness's parent-facing instructions file runs its `verify-run` postcondition check on the returning run and re-dispatches an incomplete one exactly once (change 0242). `docket-finalize-change` dispatches `docket-rebase-resolver` on a merge-gate rebase conflict and `docket-integration-repair` on a red rebased suite — also foreground, but their reports flow **back to finalize in-context** to gate the merge, acting in the feature worktree. Each dispatched agent runs at the model/effort its own wrapper resolves — literal tiers are never restated in dispatch prose. **Five** of the **seventeen** generated wrappers wrap **no skill**: `docket-auto-groom-critic`, `docket-rebase-resolver`, and `docket-integration-repair` each load only `docket-convention`; `docket-brainstorm-consultant` and `docket-plan-writer` load **no convention either** — the consultant authors prose and performs zero docket operations (ADR-0022), the plan-writer invokes the resolved plan skill at runtime as a passthrough, never as a preload, because `skills.plan` may name any installed skill. All are auto-discovered by the Go install's `agents/docket-*.md` glob.
