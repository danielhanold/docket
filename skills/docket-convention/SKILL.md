...
This 2×2 is evaluated by the same `docket-config.sh` and reported as its `BOOTSTRAP=` verdict — `PROCEED` (migrated or main-mode), `STOP_MIGRATE` (existing single-branch or half-migrated), or `CREATE_ORPHAN` (fresh). The default `--export` invocation is **read-only** (it reports the verdict); the skill acts on it — STOP and point at `migrate-to-docket.sh` on `STOP_MIGRATE`, or opt into the orphan-create write (`"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/docket-config.sh --bootstrap`, guarded to the `¬DOCKET ∧ ¬LIVE` cell) on `CREATE_ORPHAN`. The probe definitions above remain the contract the script implements.
