...
**Build model wiring.** Resolve `BUILD_IMPLEMENTER`/`BUILD_REVIEWER` from the Step-0 config export. Each, when set, fills SDD's already-required `model:` field for its dispatches — no SDD fork, just a value supplied where SDD already asks for one: `BUILD_IMPLEMENTER` for the per-task implementer subagent and any fix subagent; `BUILD_REVIEWER` for the per-task reviewer subagent and the Step 6 final whole-branch code-reviewer. When a role is unset, instruct nothing for it — SDD keeps its own Model Selection judgment for that dispatch. A set role is a deliberate blunt override of SDD's per-complexity adaptivity for that role, applied uniformly across the change.
