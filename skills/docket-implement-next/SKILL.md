...
**Build model wiring.** Resolve `BUILD_IMPLEMENTER`/`BUILD_REVIEWER` from the Step-0 config export. Each, when set, supplies the `model:` this controller passes on its build-phase dispatches — no SDD fork: `BUILD_IMPLEMENTER` covers the per-task implementer subagent and any fix subagent (SDD's own already-required `model:` field for those two genuine SDD sub-dispatches); `BUILD_REVIEWER` covers the per-task reviewer subagent (same, SDD's `model:` field) AND the Step 6 final whole-branch code-review dispatch — that one is run via the separately-resolved `$SKILL_REVIEW`, not an SDD sub-dispatch, but `BUILD_REVIEWER` fills its `model:` the same way. When a role is unset, instruct nothing for it — the dispatch keeps SDD's own (or, for the final review, its own default) Model Selection judgment. A set role is a deliberate blunt override applied uniformly across the change.

### Step 6 — Review + ADRs

The **resolved review skill** — `$SKILL_REVIEW` from the Step-0 config export (default `superpowers:requesting-code-review`) — whole-branch; on `auto` or unavailability, apply the review auto-fallback per the convention's *Skill layer* (a whole-branch review before the PR opens, warning prominently). Re-read `<changes_dir>/LEARNINGS.md` first so past lessons feed the review. For any non-obvious decision made during implementation, **dispatch the `docket-adr` subagent** (foreground, run at the model/effort its wrapper resolves) to record it (it assigns the number + updates the index) — once per decision; it commits the ADR on `origin/docket`, publishes it onto the integration branch on acceptance, and **returns the number**. After re-syncing `.docket/`, append that returned number to the change's `adrs:`. Update `adrs:` in the **metadata working tree on `metadata_branch`** (in `docket`-mode, `.docket/` on `origin/docket`) — never in the feature worktree (the change file is metadata; the same discipline as step 7). After writing `adrs:`, regenerate the change's `## Artifacts` block: `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/render-change-links.sh --change-file .docket/<changes_dir>/active/<id>-<slug>.md --adrs-dir .docket/<adrs_dir>` (the block edit rides with this same adrs-field commit; the renderer is the sole writer of the block). (`docket-adr` itself already commits the new ADR file on `metadata_branch` and, on acceptance, publishes it onto the integration branch.)
