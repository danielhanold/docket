...
When the claimed change carries `stacked_on:`, **read [`../docket-convention/references/stacked-changes.md`](../docket-convention/references/stacked-changes.md) now (blocking)** before cutting: the branch comes from the resolved effective base (`docket.sh stack-base`, invoked unconditionally) and the step-7 PR targets that base, not `origin/<integration_branch>`.
