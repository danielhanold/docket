...
**PR-body assembly.** The authored PR body carries three docket elements: the best-effort `#<issue>` reference (never `Closes #N`), the **back-link line** (`↩ Change <padded-id> — <title>`, change 0136), and the **build-evidence block** (change 0170) — the current evidence record written marker-bounded (`<!-- docket:build-evidence:start -->` / `<!-- docket:build-evidence:end -->`) alongside the review outcome, the block `docket-finalize-change` reads to decide whether its post-rebase suite run can be skipped. Before assembling any of the three, **read `references/edge-paths.md` now (blocking)** — it owns the mechanics, including marker validation and the expected-staleness rule for a post-gate `head_sha`.
