...
When the effective base merely advances under a **completed, clean** owned rebase, re-running `finalize.rebase` with the same identity forward-rebases the current head onto the new base — never a reset. Prior resolutions and committed work carry forward; conflicts take the ordinary reserve/resolve/continue flow on the attempt's remaining budget (movement replenishes nothing); the full suite re-runs under a fresh `attempt` token superseding the old.

- **Unpublished** owned rebase (`--id --version --head` unchanged): a `base-moved-under-receipt` `blocked` means the base **diverged** (was rewritten) or recorded-attempt resolver work is unsettled — retained, `halted`; never permission to reset or abort a completed rewrite.
- **After finalize published the result** — the remote feature head and open PR both name the tested head — re-entry refreshes the **same owned attempt** rather than refusing: the receipt's completed-gate checkpoint and matching local, remote, and PR heads prove the moved remote is docket's own published result, so the lease is re-keyed to it and the suite re-runs before the next exact-lease publish. Re-enter with the **current** `--version --head` from `context finalize` — the published head, never the pre-rebase head, which now fails the PR-head check. A moved remote **not** provably docket's own — a missing or mismatched checkpoint, unverifiable evidence, or foreign edits — stays retained (`remote-head-mismatch`, `blocked`): never overwritten or adopted.
...
The resolver echoes the token back as the report's `resolver_reservation` field. It edits only the conflicted regions in the returned workspace and returns a versioned `ResolverReport` (fields in step 3 of *The two agents* in `references/gate-failure.md`) — it never runs the rebase mechanics or the suite. In Docket's own repository, conflicts under `internal/assets/embedded/` are derived outputs the controller clears by regenerating the bundle: generated-only stops never reach this reserve step (the operation clears them itself, spending no budget), and on a mixed stop the resolver reports only the authored paths while `finalize.rebase-continue` regenerates and stages the bundle in the same continue.
...
- `exhausted` (`resolver-budget-exhausted`) — the configured `finalize.resolver_max_attempts` budget (default 10) is spent: `halted` via the abort flow below. Raising the limit takes effect on the next explicit finalize attempt, never this receipt.
...
Feature worktree: <absolute canonical feature-worktree root>
Repair-attempt budget: <finalize.repair_max_attempts from the Step-0 prepared context> (initial attempt included)
Red-test root cause, feature-branch fix bounded to the dispatched repair-attempt budget, and claimed commits plus `repaired`/`stuck`; it never rebases, merges, or transitions metadata. Then re-run the gate on the repaired head: `gate.launch` with `--root <run-root> --cwd <feature worktree> -- <resolved suite>`, then `gate.observe` with `<run-dir>`, under `docket-build`'s gate-execution posture. On a `passed` terminal observation whose head equals repaired head, `evidence.record --id <id> --run <absolute-run-dir> --head <repaired head>` returns the immutable block — no agent-supplied `passed` boolean; a failed/running/stopped/vanished/malformed/head-mismatched run produces none, and a repair that cannot reach green within the configured `finalize.repair_max_attempts` budget, or unavailable repair dispatch, is `halted`.
<!-- docket:feature-dispatch:end -->
