...
The copy-set: the archived change file, its `spec:` if set, and each `adrs:` entry whose ADR is `Accepted` (`Proposed`/draft ADRs are skipped). `BOARD.md` is **never** published. Mechanics — `checkout origin/docket -- <copy-set>`, the CAS push, self-verify, teardown — are owned by `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/terminal-publish.sh --id <id> --enabled <terminal_publish>` (or `terminal-publish.sh --adr <NN> --enabled <terminal_publish>` for the ADR-only path — `<terminal_publish>` is the resolved config's `TERMINAL_PUBLISH` value from `docket-config.sh --export`); see `scripts/terminal-publish.md`.
