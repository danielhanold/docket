...
**Stacked changes:** before step 1, run `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/docket.sh stack-children --changes-dir <changes_dir> --id <id>` and keep its output — that scan, never the parent's rendered `## Stacked children` row (which lags any child added after the parent's last write), tells you whether this change has stacked children, and it answers step 3.5's gate. When it prints anything, or this change carries `stacked_on:`, **read [`../docket-convention/references/stacked-changes.md`](../docket-convention/references/stacked-changes.md) now (blocking)** — it owns the rebase target, the open-children merge gate (autonomous hard-blocks; interactive warns and the human may override), the child-PR retarget owed before step 4 deletes this branch, and the stack close-out that step 3.5 runs when this change is a stack root.
...
3.5 **Close out the stack** — only when the trigger scan above printed descendants (skip it entirely otherwise; the reference's *The stack close-out is idempotent* section owns the gate, the `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/docket.sh stack-closeout` invocation, and its report-line handling — **follow it exactly**). This change's code has just reached the integration branch, so every descendant parked at `stacked-merged` is `done` by the governing invariant and this is the only moment anything will say so: `docket-status`'s sweep scans `active/` for merged PRs, and step 3 has just taken this change out of `active/` while a `stacked-merged` descendant has no merged PR of its own to be found by. Skipping it strands every descendant permanently. Pass step 3's UTC `mergedAt` as `--date`, run it **before** step 4 deletes this branch, and relay the report lines verbatim; a failed line is **log-and-continue plus a loud report naming the by-hand re-run**, never an abort — the change is already archived, and stopping here would forfeit cleanup and the board too.
...
2. **Rebase** `feat/<slug>` onto `origin/<integration_branch>`. On conflict, dispatch the `docket-rebase-resolver` subagent (foreground, at the model/effort its wrapper resolves). Name the **feature worktree** in the dispatch payload — the facade's `--worktree` gate refuses a delegated dispatch that names none. An **ambiguous conflict** it can't resolve aborts the rebase and the gate **abort-and-reports**. On any conflict, **read `references/gate-failure.md` now (blocking)** — it owns the resolver/repair split and the abort mechanics.
...
5. **Validate per `gate`:**
   ...
   - On **red**, dispatch `docket-integration-repair` (foreground, at the model/effort its wrapper resolves). Name the **feature worktree** in the dispatch payload — the facade's `--worktree` gate refuses a delegated dispatch that names none. Green → the sign-off rule below; stuck / cannot reach green in at most two attempts → **abort-and-report**, as do red or absent CI checks under `ci`/`both`. On any red, **read `references/gate-failure.md` now (blocking)** — it owns the repair bound and the sign-off gating.
