...
- **A successful finalize removes the section.** Unlike auto-groom's human-only re-arm, the condition is machine-verifiable (the gate passed), so requiring a human to delete it would strand stale markers on changes that are fine. That removal is a **live-path obligation** — an `implemented` change that stays `implemented` must not carry a stale needs-you cell — and **not** a guarantee about archived files: nothing strips the section at close-out, so on an out-of-band human merge it rides into `archive/` verbatim. Nothing needs to strip it. **Every automated reader of the marker is scoped to a change that has not yet reached `done`** (today: the board cell, the auto-detect selection skip, the `stale-finalize-blocked` health check, and the GitHub mirror's readiness label — each `implemented`-or-unmerged-only; the mirror is the one that also scans `archive/`, and its `readiness_label` has no `done` arm), so archiving retires the marker's meaning whether or not the section is physically present. The archived section does keep a **human** reader — the record of *why* a change stalled at the merge gate — and that is the point. This is the presence-encoded-state rule discharged, not waived: removal is its usual *means*, and the *end* is that no reader is left misinformed — where every automated reader has already stopped consulting the artifact, its presence encodes nothing. Do **not** add strip-on-archive to satisfy the rule literally; it would destroy that record, and put body surgery into the shared terminal primitive for zero observable gain.
