...
Commit the stub's outcome (change-file edit + spec when emitted) in the metadata working tree; push `origin/docket`. On a non-fast-forward rejection: re-run `docket.sh preflight`, and if the rebase brought in commits touching this stub's file, RE-READ it — no longer autonomous-eligible (groomed, killed, claimed, or opted out elsewhere) ⇒ DISCARD this iteration's writes for it (`git -C .docket restore -- <changed paths>` for the change-file edit, and delete the just-drafted spec file — it is this iteration's own uncommitted artifact) and loop. Then invoke `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/docket.sh docket-status --board-only` — the single Board-pass entry point; it renders, commits, and pushes `BOARD.md` itself (a separate commit, only if the board changed). **Must-land:** key on the stdout report line, not the exit code — re-invoke until it reports `board inline changed pushed`, `board inline clean`, or `board off`; on `board inline changed push-failed`, re-run `docket.sh preflight` and invoke it again. Loop to step 1.
