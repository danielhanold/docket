...
5. **Commit, push & Board pass** — commit the change + spec together (NOT `BOARD.md`) and PUSH to `origin/docket`, a **must-land** commit: retry (re-run `docket.sh preflight`, then re-push) until it lands (immediately reviewable on GitHub; visible to the autonomous implementer). Run the must-land Board pass: `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/docket.sh docket-status --board-only --must-land` — a non-zero exit means the board did not land; STOP and surface it (abort-and-report). STOP. Never implements.
