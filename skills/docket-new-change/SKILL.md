...
5. **Board, commit & push** — invoke `"${DOCKET_SCRIPTS_DIR:?run docket/install.sh}"/board-refresh.sh --changes-dir .docket/<changes_dir> --surfaces "$BOARD_SURFACES"` (the gated writer resolved at Step 0), then stage `BOARD.md` and commit it together with the change + spec — **only if BOARD.md changed** (`git status --porcelain -- <changes_dir>/BOARD.md` is non-empty; inline disabled or an unchanged board leaves the file untouched, a genuine no-op) — and PUSH to `origin/docket`, a **must-land** commit: retry (`pull --rebase`) until it lands (immediately reviewable on GitHub; visible to the autonomous implementer). STOP. Never implements.
