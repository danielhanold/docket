...
Selection is ordered by *mergeability* rather than priority — `depends_on` order first (a hard constraint), then GitHub's `mergeable`, then the smallest diff, with priority → age → id as the tiebreak — so each drain lands as many changes as it can before anything stops it. A change whose gate fails is marked with a `## Finalize blocked` section (dated in its body), shows on the board as **finalize blocked — needs you**, and is skipped by later runs until a successful finalize clears it automatically. As with the implementer, confirm `/loop` composes cleanly in your own harness before relying on it unattended.
